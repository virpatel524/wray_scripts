...
for alpha in range(1, len(item_dict.keys()) + 1):
	if alpha % 10 == 0:
		current_number+=1
	for listsel in itertools.product(klist, lowerwlist, dlist, clist):	
		with open('/home/vdp5/scripts/cambodia_PV01_improve_align/fastqgz_to_sam_cambodia_%d_improve.sh' %(current_number), 'a') as abouttoputin:
			...
			abouttoputin.write('bwa mem -M -k %f /home/vdp5/data/salvador_vivax_asia_2016/first-SAMEA2376790/pvivax_sal1_SAMEA2376790.fasta %s %s | samtools view  -Sb - | samtools sort - /home/vdp5/data/cambodia_samples/sequences_bam/%s.sorted.bam\n\n' %(item_dict[beta][0], item_dict[beta][1], beta.split('.')[0]))